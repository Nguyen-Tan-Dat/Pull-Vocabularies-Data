...
//        Test.writeTopics(data, "Vocabulary in use Advanced");
    ...
    private static void writeAcademicTopics() {
        var ens = Test.databaseEnglish();
        String filePath = "Cambridge vocabularies in use data/Academic.txt";
        HashMap<String, Set<String>> vocabMap = readAcademicVocabularies(filePath);
        HashSet<String> exist = new HashSet<>();
        HashSet<String> unexist = new HashSet<>();
        ArrayList<Object> data = new ArrayList<>();
        int count = 0;
        ...
        HashMap<String, Object> row = new HashMap<>();
        row.put("name", "Academic");
        row.put("vs", exist);
        data.add(row);
        Test.writeTopics(data, "Academic in Use");
    }
...
//        Test.writeTopics(data, "Vocabulary in use Pre-Intermediate 46");
    ...
    public static HashMap<String, Set<String>> readAcademicVocabularies(String filePath) {
        HashMap<String, Set<String>> unitMap = new HashMap<>();
        try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            String line;
            int index=0;
            while ((line = br.readLine()) != null) {
                ...
                try {
                    ...
                    for (String u : unit.split(",")) {
                        try {
                            String unitName = parseTopic("Academic", Integer.parseInt(u.trim()));
                            unitMap.computeIfAbsent(unitName, k -> new HashSet<>()).add(word);
                        } catch (NumberFormatException e1) {
                            System.out.println("Lỗi unit: " + unit);
